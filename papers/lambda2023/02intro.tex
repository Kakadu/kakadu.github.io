...
%  \[
%  (\lambda \tikz\node[na](word6){x}; . (\lambda \tikz\node[na](word7){y};.  \tikz\node[na](word8){y}; )) (\lambda \tikz\node[na](word9){y}; .  \tikz\node[na](word10){y}; )
%  \]
%  \begin{tikzpicture}[overlay]
%    \path[draw,->,red,thick] (word8.north) -- ++(0,0.3cm) -- ++(-0.3cm,0) -- (word7.north);
%    \path[draw,->,blue,thick] (word10.north) -- ++(0,0.3cm) -- ++(-0.3cm,0) -- (word9.north);
%  \end{tikzpicture}
%
%  %\vspace{1em}
%  Индексы: $ \lam{}{\lam{}{0}}\lam{}{0}$\\
%  Уровни: $ \lam{}{\lam{}{1}}\lam{}{0}$
%
%  Стрёмно: одинаковые лямбда-выражения в разных местах представлены по-разному
\end{frame}
...
struct ulc *evalApplyByValue(Strategy *this, ulc *f, ulc *arg) {
  auto f2 = applyStrategy(this, f);
  switch (f2->tag) {
    case VAR:    case APP:      return app(f2, arg);
    case ABS: {
      auto arg2 = applyStrategy(this, arg);
      auto rez = subst(arg2, f2->body.Abs.name, f2->body.Abs.body);
      return applyStrategy(this, rez);
    }
  }
  assert(false); return nullptr; // unreachable
}
