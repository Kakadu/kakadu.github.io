...
\begin{columns}[T]
  \begin{column}{.55\textwidth}
     ...
     \begin{itemize}
     	...
     	\item \enquote{Если это утверждение ложно, то ...} (парадокс Х.~Карри)
...
Придумать \textbf{конечный} набор недвусмысленных правил, таких что обычный офисный бюрократ (читайте, компьютер) мог им следовать и получать ожидаемый результат \textbf{за конечное число шагов}.\\

\vspace{1em}
Существуют много различных формализаций:
\begin{itemize}
\item $\lambda$-исчисление
\item Машины Тьюринга\footnote{\foreignquote{english}{The Annotated Turing} by Charles Petzold. %Считать А.Тьюринга изобретателем компьютеров~--- неправильно
}
\item Машины Поста
\item Частичные рекурсивные функции (англ. partial recursive functions)
...
\framesubtitle{Тут бывают грабли, см. разд.~\ref{sec:CAS}}
 \begin{figure}[t]
...
\end{itemize}
  ...
  {\Large
    ...
    \begin{tikzpicture}[overlay]
      ...
      \path[->,red,thick,draw] (dbI2.south) -- ++(0,-0.2cm) -- ++(0.4cm,0) -- (dbI3.south);
    \end{tikzpicture}
  }
   \end{minipage}
...
\def\Z{\ensuremath{\mathbb{0}}}
\def\I{\ensuremath{\mathbb{1}}}
\def\II{\ensuremath{\mathbb{2}}}
\def\IV{\ensuremath{\mathbb{4}}}
...
$ \I \sim \lam{f}{\lam{x}{\ f\ x}}$

$ \II \sim \lam{f}{\lam{x}{\ f\ (f\ x)}}$
...
\begin{frame}{Вычисление (т.е. редукция, упрощение) \II+\II}
\def\betaarr{\longrightarrow\!\!\beta}
\def\numTWO{\ensuremath{\lam{f}{\lam{x}{f(f\ x)}}}}
\vspace{-2em}
{\Large
  \begin{align}
    \App{\lam{m}{\lam{n}{\lam{f}{\lam{x}{\app{m\ f\ }{\app{n\ f\ }{x}} }}}}}{\II} \II & \longrightarrow \\
    \Redex{\lam{m}{\lam{n}{\lam{f}{\lam{x}{\app{m\ f\ }{\app{n\ f\ }{x}} }}}}}{\II} \II & \betaarr \\
    \redex{\lam{n}{\lam{f}{\lam{x}{\app{\II\ f\ }{\app{n\ f\ }{x}}){}}}}} {\II} & \betaarr \\
    \lam{f}{\lam{x}{ \app{\II\ f\ }{\app{\II\ f\ }{x}}}}   & \longrightarrow \\
    \lam{f}{\lam{x}{ \App{\Redex{\numTWO}{f}\ }{\app{\II\ f\ }{x}}}} & \betaarr \\
    \lam{f}{\lam{x}{ \App{\lam{x}{f (f x)} \ \ }{\app{\II\ f\ }{x}}}} & \longrightarrow \\
    ...
    \lam{f}{\lam{x}{ f (f (f (f x)))}} & \equiv \IV
  \end{align}}
\end{frame}
...
\begin{frame}[plain]{Вычисление \\(т.е. редукция, упрощение) \\$\II\times\II$ длинно}
\def\betaarr{\longrightarrow\!\!\beta}
\def\numTWO{\ensuremath{\lam{f}{\lam{x}{f(f\ x)}}}}
\setcounter{equation}{0}
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.55\textwidth}
        ...
        {  \begin{align}
          \apppl{\app{\lam{m}{\lam{n}{\lam{z}{\lam{x}{(m \app{n}{z} x)}}}}}{\II}}{\II} & \longrightarrow \\
          \Apppl{\Redex{\lam{m}{\lam{n}{\lam{z}{\lam{x}{(m \app{n}{z} x)}}}}}{\II}}{\II} & \betaarr \\
          \Redex{\lam{n}{\lam{z}{\lam{x}{(\II \app{n}{z} x)}}}}{\II} & \betaarr \\
          \lam{z}{\lam{x}{\app{\app{\II}{\app{\II}{z}}}{x}}} & \longrightarrow \\
          \lam{z}{\lam{x}{\App{\Redex{\numTWO}{\app{\II}{z}}}{x}}} & \betaarr \\
          \lam{z}{\lam{x}{\Redex{\lam{x}{(2 z (\app{\II}{z} x))}}{x}}} & \betaarr \\
          \lam{z}{\lam{x}{\App{\app{\II}{z}}{(\app{\II}{z} x)}}} & \longrightarrow \\
          \lam{z}{\lam{x}{\app{\Redex{\numTWO}{z}}{(\app{\II}{z} x)}}} & \betaarr \\
          \lam{z}{\lam{x}{\redex{\lam{x}{\app{z}{\app{z}{x}}}}{(\app{\II}{z} x)}}} & \betaarr \\
          \lam{z}{\lam{x}{\app{z}{\app{z}{\App{\app{\II}{z}}{x}}}}} & \longrightarrow \\
          \lam{z}{\lam{x}{\app{z}{\app{z}{\App{\Redex{\numTWO}{z}}{x}}}}} & \betaarr \\
          \lam{z}{\lam{x}{\app{z}{\App{z}{\Redex{\lam{x}{\app{z}{\app{z}{x}}}}{x}}}}} & \betaarr \\
          \lam{z}{\lam{x}{\app{z}{\app{z}{\app{z}{\app{z}{x}}}}}} & \equiv \IV
        \end{align}}
  \end{subfigure}
\end{figure}
...
Например:
\begin{itemize}
\item $\lam{z}{\lam{x}{\app{z}{\app{z}{\app{z}{\app{z}{x}}}}}}$ и $\lam{f}{\lam{x}{\app{f}{\app{f}{\app{f}{\app{f}{x}}}}}}$ эквивалентны \IV{}
\end{itemize}
...
\end{definition}
В этом смысле quick sort эквивалентен сортировке пузырьком
\end{frame}
...
F\equiv \lam{x}{\lam{y}{y}}\equiv snd \equiv \Z{}
\]

\only<1>{
Вариант 1: условие $c$ уже известно
\begin{align*}
  & \app{\app{c\ }{th}}{el}\\
  & \app{\app{c\ }{th}}{el} \arrbeta th \\
  & \app{\app{c\ }{th}}{el} \arrbeta el
\end{align*}

}
\only<2>{
Вариант 1: условие не известно, нужно делать if-then-else
\begin{align*}
...
\begin{frame}{Напоминание из алгебры}
\begin{definition}[Неподвижная точка оператора $R$]
такой аргумент $x$, что $Rx = x $
\end{definition}
\vspace{2em}
Пример. Мы ищем все достижимые клетки-локации в лабиринте. Оператор $R$ будет делать делать шаг из всех достижимых на данном этапе клеток во все возможные стороны.\\
Неподвижной точкой будет множество достижимых клеток, такое что других достижимых нет.
\end{frame}
...
\newcommand{\FAC}{\ensuremath{\lam{\self}{\lam{n}{ \ite{n<\II{}}{\I{}}{n \times \self(n-\I)}}}}}
%Основное свойство
\[
\Y R = R (\Y R)
\]
...
\begin{align*}
  \Yfac\ \II{} & \equiv \\
  \redex{\Y}{\FAC}\ \II{} & \arrmany\\
  \redex{\FAC}{\Yfac}\ \II{}  & \longrightarrow\\
  \redex{(\lambda n . \ite{n<\II{} }{\I{}}{n \times \Yfac (n-\I{})})}{\II{} } & \arrmany \\
  \II{}  \times \Yfac (\II{} - \I{}) & \betaarr \\
  \II{}  \times (\Yfac \I{}) & \arrmany \\
  \II{}  \times \ite{\I{}<\II{}}{\I{}}{n \times (\Y\ f\!ac\ (\I{}-\I{}))} & \arrmany \\
  \II{}  \times \I{} & \arrmany \II{}
\end{align*}
 \end{frame}
