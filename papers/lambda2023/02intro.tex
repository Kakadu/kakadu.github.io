...
\begin{frame}{Проблемы неформального определения}
	\begin{itemize}
		\item Зависимо от естественного языка
		\item Не совсем понятно, что является допустимой операцией, а что нет.
		\begin{itemize}
...
\begin{figure}[ht]
  ...
  \vspace{-8em}
  Первым ввел аксиоматику арифметики в 1889 году. Натуральные числа определяются через \textbf{\enquote{базу}} и \textbf{\enquote{следующий}}\\
...
$ 1 \sim \lam{f}{\lam{x}{\ s\ x}}$

$ 2 \sim \lam{f}{\lam{x}{\ s\ (s\ x)}}$
...
\begin{frame}[fragile]{Подстановка}
 \begin{figure}[t]
   ...
   \vspace{-5em}
    \begin{definition}{Редекс}
      --- это $\lambda$-выражение вида $(\lambda x. B)A$
    \end{definition}
...
\begin{frame}{Вычисление (т.е. редукция, упрощение) 2$\times$2}
\def\betaarr{\longrightarrow\!\!\beta}
\vspace{-2em}
\begin{align*}
  \lam{m }{\lam{n}{\lam{z}{\app{m}{\app{n}{z}}}}} 2 2  & \longrightarrow \\
  \tb{\lam{m }{\lam{n}{\lam{z}{\app{m}{\app{n}{z}}}}}} \tr{2} 2  & \betaarr \\
  \tb{\lam{n}{\lam{z}{\app{2}{\app{n}{z}}}}} \textcolor{red}{2}  & \betaarr \\
  \lam{z}{\tb{2} \textcolor{red}{\app{2}{z}}}  & \betaarr \\
  \lam{z x}{\app{\tb{2} \tr{z}}{(2 z x)}}  & \betaarr \\
  \lam{z x}{\textcolor{blue}{\lam{x}{\app{z}{\app{z}{x}}}} \tr{(2 z x)}}  & \betaarr \\
  \lam{z x}{\app{z}{\app{z}{\app{\tb{2} \tr{z}}{x}}}}  & \betaarr \\
  \lam{z x}{\app{z}{\app{z}{\textcolor{blue}{\lam{x}{\app{z}{\app{z}{x}}}} \tr{x}}}}  & \betaarr \\
  \lam{z x}{\app{z}{\app{z}{\app{z}{\app{z}{x}}}}} &  \equiv 4
  \end{align*}
\end{frame}
...
\begin{align*}
  ...
  (ite\ T) \quad\equiv &\quad  \lambda t. \lambda e. (T\ t\ e) \arrmany \lam{t}{\lam{e}{t}} \equiv T\\
  (ite\ F) \quad\equiv &\quad  \lambda t. \lambda e. (F\ t\ e) \arrmany \lam{t}{\lam{e}{e}} \equiv F
\end{align*}
...
\begin{frame}{Факториал с помощью комбинатора неподвижной точки (сокращённо)}
\vspace{-1em}
...
Факториал: $fac \equiv \FAC$\\

\begin{align*}
  \FIX \FAC 2 & \longrightarrow\\
  (\lambda n . \ite{n<2}{1}{n \times \FIX\ f\!ac\ (n-1)}) 2 & \arrmany \\
  2 \times \FIX\ f\!ac\ (2-1) & \arrmany \\
  2 \times (\FIX \FAC\ 1) & \arrmany \\
  2 \times \ite{1<2}{1}{n \times (\FIX\ f\!ac\ (1-1))} & \arrmany \\
  2 \times 1 & \arrmany 2
\end{align*}
 \end{frame}
