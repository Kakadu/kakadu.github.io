...
* Backend
    ...
    * Объединить порождение кода для RISC-V и AMD64.
      ...
      Сейчас там по сути две независимые реализации. Как вариант, можно повторить [C-- как в OCaml](https://github.com/ocaml/ocaml/blob/5.2.0/asmcomp/cmm.mli). Про это всё есть статьи, но такое ощущение, что лучше  подсмотреть представление в реальном коде и сделать как в OCaml.
    * Интересно сделать для OCaml производительный JIT.

      Сейчас есть способ компиляции OCaml в стековый байт код, который уже JITтили, и получали ускорение в 2 раза. К сожалению, это примерно в 5 раз медленнее, чем компиляция OCaml в native код. По-видимому, вначале надо скомпилировать  OCaml в регистровый байткод (JVM, .NET или чего попроще), и джиттить уже эту абстрактную машину.
...
* TODO: https://player.vimeo.com/video/833791219#t=3h42m48s
  ...
  Думаю, что второкурсникам, не знакомым с OCaml, абстрактные темы ещё не по зубам. В таком случае есть более "конкретные" темы.
