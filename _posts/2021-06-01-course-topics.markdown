...
Для поиска повтор программного кода (в проторечье -- "копипаста")  нужны специализированные тулы. Они разбюиваются на те, что анализируют текст как он есть, и на те, которые его специфическим образом предобрабатывают (разбивают на лексемы, анализируют деревья, переименовывают переменные единым образом перед анализом и т.п.).

Сейчас для курса по "Функциональному программированию" на OCaml используется утилита...
...
Но возможно стоит поискать другие утилиты, напрмеир https://pmd.github.io/pmd/index.html. Там нет поддержки OCaml, но я не хочу писать на Java.
{% endcomment %}
