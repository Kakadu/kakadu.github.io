...
* Backend
    ...
    * [Интеграция с настоящим райнтаймом языка OCaml.](#ocaml-LLVM-runtime)
      ...
      Гипотетически, тема развивается в "скрещивание рантайма языка OCaml  с LLVM". Последний раз когда пытались сделать для OCaml порождение кода c помощью LLVM, получалось очень неэффективная реализация, потому что LLVM был не готов: shadow стек для сборки мусора приводил к тому, что компилятор без LLVM порождал более эффективный код. За десяток лет LLVM улучшился, и, может быть, сейчас всё стало гораздо лучше?
