...
По некоторым темам консультантом буду выступать не только я. Формального руководителя как-нибудь найдем. Темы могут быть завершены, сделаны частично или не начаты.
...
**Для**: уже знакомых с ФП

Для стимулирования разработки компиляторов на кафедре СП матмеха, запущен проект про разработке ещё одной реализации компилятора OCaml.
На данный момент он имеет только официальный компилятор, и наличие второй реализации может плодотворно сказаться на стандартизации.

Текущий прототип находится [здесь](https://github.com/Kakadu/rukaml). Там есть
...
* Front
    * Другой парсер с векторизацией а ля SIMDJSON
    * Алгебраических типов пока совсем нет. Скорее всего вестно 2025 они уже появятся.
      * GADT разумеется тоже нет
    * Рекурсивные значения?
    * Модули/функторы?
      * Поддержка программ, разбитых на файлы. (Частично сделано)
    * Реализовать промежуточное представление программы с помощью CPS. **Занято**

      Если так сделать, то должны включиться интересные оптимизации, свойственные ФП. Про это можно найти много литературы
      * [Conversion by Hirrolot](https://gist.github.com/Hirrolot/d16dc5e78639db6e546b5054afefd142) Скорее всего самый простой вариант
        * [Evaluation by Hirrolot](https://gist.github.com/Hirrolot/3b233b7a80edf43234d2b89051af12b4)
      * https://wingolog.org/archives/2023/05/20/approaching-cps-soup
      * Reasoning about Programs in Continuation-Passing Style 1993
        https://citeseerx.ist.psu.edu/doc/10.1.1.22.7509
      * [No-Brainer CPS Conversion (Functional Pearl)](https://www.khoury.northeastern.edu/home/shivers/papers/nobrainer-cps.pdf) by Milo Davis, William Meehan, and Olin Shivers

      На данный момент, там простой и наивный ANF вместо промежуточного представления. Почти никаких оптимизаций нет. Было бы круто, сделать что-то такое, что даст ощутимую разницу [вот для  этого примера](https://discuss.ocaml.org/t/manual-cps-is-faster-than-cps-monad-is-it-expected/13560).

* Front+Back
    * external
    * массивы + автовекторизация

      Присваивания ни в каком виде сейчас нет.
      При этом можно сделать
      * relaxed value restriction как в OCaml,
      * или что-то более навороченное.
    * Алгебраических типов пока нет
    ...
    * multicore
    * Простой inline (с примерами отсюда: [https://ocamlpro.com/blog/2025_02_19_the_flambda2_snippets_4](https://ocamlpro.com/blog/2025_02_19_the_flambda2_snippets_4))
      * Или на тему специализации: [https://ocamlpro.com/blog/2018_04_01_ocaml_jtrt](https://ocamlpro.com/blog/2018_04_01_ocaml_jtrt)
    * OOP
...
* Backend
    * Выделения памяти на стеке и хвостовая рекурсия.
      ...
      Никакой поддержки хвостовой рекурсии сейчас нет. Для уважающего себя функционального языка она необходимо.
      Также стоило бы рядом реализовать аннотации (@@local), чтобы выделять некоторые данные на стеке (для простоты -- n-ки). Эта идея не с потолка, этим действительно [занимаются](https://antonlorenzen.de/oxidizing-ocaml-modal-memory-management.pdf).
    ...
    * [Интеграция с настоящим рантаймом языка OCaml.](#ocaml-LLVM-runtime)

      А именно, порождать код так, чтобы можно было переиспользовать уже имеющийся рантайм. Для этого придется разобраться в текущих соглашения о вызовах языка OCaml (они плохо документированы) и написать свой генератор кода, вероятно ещё по дороге хакерски изменяя рантайм OCaml.

      Гипотетически, тема развивается в "скрещивание рантайма языка OCaml  с LLVM".
      Последний раз когда пытались сделать для OCaml порождение кода c помощью LLVM, получалась очень неэффективная реализация, потому что LLVM был не готов: shadow стек для сборки мусора приводил к тому, что компилятор без LLVM порождал более эффективный код.
      За десяток лет LLVM улучшился, и, может быть, сейчас всё стало гораздо лучше?

    * Исследование альтернативных представлений данных для языка программирования OCaml.

      Сейчас используются "теггированные intы", интересно понять. какой эффект будет, если базовым представлением значений будет double ([NaN boxing](https://piotrduperas.com/posts/nan-boxing) как в V8)
    * Объединить порождение кода для RISC-V и AMD64.

      Сейчас там по сути две независимые реализации. Как вариант, можно повторить [C-- как в OCaml](https://github.com/ocaml/ocaml/blob/5.2.0/asmcomp/cmm.mli).
      Про это всё есть статьи, но такое ощущение, что лучше подсмотреть представление в реальном коде и сделать как в OCaml.
    * Интересно сделать для OCaml производительный JIT.

      Сейчас есть способ компиляции OCaml в стековый байт код, который уже JITтили, и получали ускорение в 2 раза. К сожалению, это примерно в 5 раз медленнее, чем компиляция OCaml в native код.
      По-видимому, вначале надо скомпилировать OCaml в регистровый байткод (JVM, .NET или чего попроще), и джиттить уже эту абстрактную машину.
    * ARM64 backend (for Apple)
...
* TODO: [Leo White's talk](https://player.vimeo.com/video/833791219#t=3h42m48s)

  Наверно, 3курсниками ПИ, которым сдавать в 5 семестре курс по компиляторам, это будет более интересно.
  Думаю, что второкурсникам, не знакомым с OCaml, абстрактные темы ещё не по зубам.
  В таком случае есть более "конкретные" темы.

{: #ocaml-linter }

## OCaml linter (a.k.a. zanuda)

Код на языках прораммирования можно пытаться проверять разными способами и утилитами. Те проверки, которые не делает компилятор часто реализуют с помощью так называемых "линтеров". Для языка OCaml на языке OCaml мною [написан](https://github.com/kakadu/zanuda) такой линтер, он применяется для проверки домашних заданий обучающихся на 2м курсе ПИ.

Сейчас он кое-что умеет, но есть куда развиваться.
Есть небольшое количество инфраструктурных [задач](https://github.com/Kakadu/zanuda/issues/23),  а также [большое количество](https://github.com/Kakadu/zanuda/issues/3) проверок ("линтов"), которые пока не реализованы. Мне нужно найти человек(-а), который реализует большее количество линтов.

По сути задача разбивается на большое количество небольших задач по реализации (иногда исправлению) проверок.
Так как из работы сложно выделить цельную большую задачу, то она **плохо подходит под ВКР**.
(Но если Вы найдете формального научника, которых убедит меня, что всё хорошо -- посмотрим.)
Лучше всего это делать как набор **полугодовых** учебных практик.
Публикация сомнительна (если только после набора большой и интересной статистики; в местах, где обсуждается обучение ФП (например, workshop TFPIE)).
Количество линтов/задач, которые нужно реализовать зависит от вредности комиссии, которой вы будете это сдавать.
...
**Для** тех, кто хочет познакомиться с С++/Qt.

Проект по open-source ремейку (уже не продающейся) игры "Космические рейнджеры 2: Доминаторы.Перезагрузка" давно заброшен, но при этом ещё [собирается](https://github.com/OpenSRQt/OpenSR), и если ему подсунуть ресурсы от уже не продаваемой версии.
Прототип написан на Qt/QML 6, работает под Linux (под остальным не пробовал).
Самой игры сейчас нет, только набросок движка. Реализовывать можно [в том порядке, в каком нравится](https://github.com/OpenSRQt/OpenSR/issues/1).
Что именно можно делать найдете в каком-нибудь Let's Play.

**Тестовое задание**

* Собрать у себя на компьютере. В CI оно собирается, можно там посмотреть как.
* Добыть (например, найти в Стиме HDпереиздание) и распаковать ресурсы для старой версии
  * Частично они извлекаются скриптами файла `Makefile.root`
  * Частично нужно после сборки запустить распаковщик ресурсов и распаковать в нужную директорию.
    В [старой вики](https://github.com/ObKo/OpenSR/wiki/Original-Game-Resources---ru) можно прочитать, что да как
* Запустить. Новая игра -> Следующий ход и увидеть как разлетаются в разные стороны астероиды.
...
## Создание OOXML Word документов

Необходима библиотека для программногоо создания MsWord документов.
Самыми лохматыми сейчас вроде бы являются реализации для [Python](https://python-docx.readthedocs.io/en/latest/) и .NET, хочется что-то на С++ и/или OCaml (новая библиотека или доделанная текущая).
...
[SAIL](https://github.com/rems-project/sail) --- эмулятор процессоров, который запускается на пользовательских машинах.
Принимает на вход свой язык (тоже SAIL) описания архитектуры, и компилирует это в эмулятор на Си или OCaml.
(Например, [вот модель описания архитектуры процессоров RISC-V](https://github.com/riscv/sail-riscv)).
Сам компилятор эмуляторов написан на OCaml.
...
Темы до магистерской включительно. В 2022 году защищалась уже [одна магистерская](https://docs.yandex.ru/docs/view?url=ya-disk-public%3A%2F%2F%2FduGmdaq1xtulqF%2BeYf1LeGLE7lWW50BvsDTUT7i2hppQ0SKmrQXcO7CWO%2Fi%2Fhy4q%2FJ6bpmRyOJonT3VoXnDag%3D%3D%3A%2FBalashov-report.pdf&name=Balashov-report.pdf&nosw=1) на эту тему.
...
Система сборки проекта Embox состоит из мешанины скриптов и Makefileов, которая задает некоторый язык описания проектов, но который на данный момент сложно расширять.
Предлагается сделать систему сборки, которая понимаеттекущую кодовую базу, но более вменяемая.
...
В мире OCaml всё странно с проектированием GUI.
Много фреймворков реализуют GUI с помощью реактивного программирования, но они специфичные для OCaml.
Из общеприменимого можно называть Melange/ReasonML, который включает в себя поддержку программирования для Web с помощью ReactJS.
Но HTML-движок имеет свои недостатки, в частности размер образа и производительность, по этому появились проекты React Native,
[Revery](https://github.com/revery-ui/revery) (свой нативный фреймворк для десктопа, где входной синтаксис повторяет проектирование на ReasonML для React, на данный момент заброшен)
и [Bogue](https://github.com/sanette/bogue) (чисто OCaml, SDL2, GPU acceleration, по сути клон WinForms/Delphi/Swing).

Qt/QML остался без внимания. Это фреймворк для проектирования кроссплатформенных приложений для десктопа и смартфонов.
Если не вдаваться в детали, то в нём три части: язык разметки, скрипты на JavaScript для написания простых преобразований GUI, и большое количество библиотек на С++.

Цель работы: скрестить Qt/QML и OCaml.
Из этого можно выделять задачи и подзадачи на неопределенное количество человек.


**частично сделано**
- Хочется, чтобы была возможность описывать всё (GUI и бизнес-логику) не уходя сильно от языка OCaml.
Сейчас в Qt/QML GUI описывается на языке QML, хотелось бы, чтобы всё переехало внутрь OCaml. Как вдохновение можно посмотреть как совмещается Javascript и HTML тэги (так называемый [JSX](https://reactjs.org/docs/introducing-jsx.html)).
...
- Это [частично сделано](https://github.com/mikhaylovilya/ocaml-syntax-extension) в рамках [учебных практик](https://se.math.spbu.ru/thesis_download?thesis_id=1234).
...
В зависимости от задач можно защищать как разное.
Публикабельность пока не понятна, если удастся удачно встроиться в систему типов OCaml и навернуть типизацию -- может быть.
...
Можно заниматься менее абициозными задачами, а именно обязать пользователя писать на QML на QML, а логику на OCaml.
Такой подход предлагается в проекте [lablqml](https://github.com/Kakadu/lablqml/issues/70).
Там осталось некоторое количество не очень сложных задач, я вижу их как ознакомительные или вводные для постепенного погружения в мир Qt/QML и OCaml.
По сути надо взять некоторые прикольные особенности QML последних версий и поддержать примеры с оными в lablqml.
...
ANTRL --- генератор синтаксических анализаторов, написанный на Java. Он позволяет генерировать код для [большого количества языков](https://github.com/antlr/antlr4/blob/master/doc/targets.md). Было бы круто, чтобы OCaml в этом списке тоже был.

При этом можно научиться соединять OCaml со сгенерированным кодом для  С++, или делать полноценный backend (предпочтительно).

Отчасти реализовано, но надо доделывать. https://github.com/p1onerka/antlr4-OCamlTarget

{: #ocaml-cpd }

## OCaml copy-paste detector

Для поиска повтор программного кода (в проторечье --- "копипаста") нужны специализированные тулы.
Они разбюиваются на те, что анализируют текст как он есть, и на те, которые его специфическим образом предобрабатывают (разбивают на лексемы, анализируют деревья, переименовывают переменные единым образом перед анализом и т.п.).

Сейчас для курса по "Функциональному программированию" на OCaml используется утилита... никакая, но была jscpd...
Обосновать отказ от Javascript я хотел с помощью [Sabotage: Code added to popular NPM package wiped files in Russia and Belarus](https://arstechnica.com/information-technology/2022/03/sabotage-code-added-to-popular-npm-package-wiped-files-in-russia-and-belarus)

Возможно стоит поискать другие утилиты, например CPD: https://pmd.github.io/pmd/index.html.
Там нет поддержки OCaml, но я не хочу писать на Java.

# Недооформленные/идеи

{: #chicken-miniKanren }

Адаптировать реализацию [faster-miniKanren](https://github.com/michaelballantyne/faster-minikanren) c ChezScheme/Racket на Chicken scheme
...
Хочется проврить, на сколько будет отличаться производительность у простого компилятора Chicken по сравнению с мощным Chez.
Если окажется, что не сильно, то это означает, что для miniKanren язык Scheme очень удачен.
Если сильно --- значит Chez очень крут.
...
Библиотека ASmJIT позволяет порождать на ходу ассемблер, чтобы его потом исполнять.
Применяться может, очевидно, для написания JIT.
На данный момент RISC-V никак не поддерживается, потому что "нет заказчика".

[Issue](https://github.com/asmjit/asmjit/issues/430)

## Настройки Firefox user.js

Хочется сделать тул, который умел бы сам накатывать и управлять настройками Firefox через user.js.
Неплохо бы иметь

* Приложение, которое показывает настройки, фильтрует их, дает какую-то доку и т.п. Пример консольного интерфейса можно посмотреть в [GitHub TUI](https://github.com/chshersh/github-tui/blob/main/images/demo.gif)
* Настройки нужно уметь накатывать с чистого листа на свежий профайл, и, открывать там какие-нить сайте для тестирования, что настройка что-то сломала или не сломала.

Сами настроек там очень много, на сейчас все они накидываются вперемешку и скриптами. Не совсем понятно, если убрать настройки из user.js, то отменится ли она без дополнительных действий. Примеры различных настроек.
...
Можно что-то взять из придуманного Tarides в инете: https://github.com/tarides/hackocaml/issues

{: #simd-json }

Проект https://github.com/simdjson/simdjson сделан для ускорения парсинга JSON м помощью интринскиов процессора. Сейчас есть SIMD Х86/64 и через ARM расширения.
Не хватает поддержки RISC-V с интринсиками.
...
Требудется улучшить поддержку плагина Timelines для системы ведения заметок Obsidian.
Суть таймлайнов -- раскладка событий по шкале времени и визуализация этой шкалы.
Может быть полезно для сопоставления различных событий в разных точках планеты на одной шкале времени.
С этим имеющийся плагин справляется.

Хочется доработать плагин в сторону обсуждения имеющихся событий на таймлайне.
Например, выбирается некоторая "интерпретация", которая объясняет имеющиеся события в парадигме данной интепретации.
Интересным это становится, когда появляются две конкурирующие интепретации, которые объясняют одни и те же события очень по-разному.
Каноничным примером здесь будут исторические события, например, конкурирующие интепретации двух сторон, имеющих территориальные конфликты.
Или, если отказаться от привязки событий ко времени, то можно выписать особенности некоторого языка программирования, и проинтерпретировать их с точки зрения двух конкурирующих парадигм.

Предлагается взять имеющийся плагин для таймлайнов в Obsidian и доработать его.
А именно добавить поддержку интепретаций. Сделать в интерфейсе выбор интерпретации и совместное отображение событий и интерпретаций, для их обозрения. В случае наличия места на экране, то отображение и событий, и двух конкурирующих интерпретаций.

(Официальный плагин поддерживается так себе. Экспериментировать надо с вот этим [форком](https://github.com/ReconVirus/obsidian-timelines)). Тут написано [как их дебажить](https://mnaoumov.wordpress.com/2022/05/10/how-to-debug-obsidian-plugins/). А вот [ссылка](https://docs.obsidian.md/Home) как создать первый плагин и запустить его.
...
TODO: Check https://github.com/ahrefs/atd/tree/master/atdgen
{% endcomment %}
...
Частенько, для оптимизации программы нужно её профилировать, чтобы найти места, где оптимизация принесет максимум пользы. Для реализаций miniKanren на Scheme уже [разработан](https://icfp22.sigplan.org/details/minikanren-2022-papers/4/Fail-Fast-and-Profile-On-Towards-a-miniKanren-Profiler) некоторый профилятор, хочется повторить эту работу для [реализации](https://github.com/PLTools/OCanren) miniKanren на OCaml. Ожидается, что профилятор будет подсвечивать наиболее и наименее "горячие" унификации в коде. В перспективе собирать какую-то статистику о реляциях, которые профилируются, например, как часто тот или иной аргумент является свободной переменной и т.п.

С технической стороны, стоит взглянуть на [проект](https://github.com/aantron/bisect_ppx) для подсчета покрытия код тестами. Скорее всего, нужно будет реализовать новый вид инструментализации, который будет внедрять в код реляций функции для замеров/подсчетов.
...
# Реинжениринг (отчасти) графики старых игр

Уже не поддерживающиеся игры постепенно умирают и перестают запускаться.
Чтобы быть готовым делать ремейки, стоит уметь распаковывать графические ресурсы и уметь их рендерить.
Например, для Civilization 4,5,6 есть специальные windows-only тулы, написанные на .NET, которые умеют распаковывать ресурсы. [CivNexus6](https://forums.civfanatics.com/resources/civnexus6.25987) и [NexusBuddy2](https://forums.civfanatics.com/threads/nexus-buddy-2-granny-editor-for-civ-5.496855).
Их исползование не вполне тривиально: нужно отдельно распаковывать скелет, текстуры и анимацию; скелет окажывается не в современном формате, текстуры не накладываются в Blender правильным образом или анимация просто не работает.

Хочется, например для Civilization Beyond Earth, сделать *кроссплатформенный тул*, которые позволяет рендерить графику из игры.
Можно написать всё на голых С++/OpenGL/Vulkan, или воспользоваться чем-то, сделанным в библиотеке Qt3D.
Рендеринг юнитов скорее всего будет проще. С ландшафтами ожидаю больше проблем.
...
## Совсем неформально описанное и идеи

- Frontend для Haskell на OCaml.
  * Это может дать более легковесный фронтенд, былее быстро компиляющией
- Транслять из SML в OCaml
  * Апробировать на кодовой бази Mlton
  * Позволит проще анализировать старые кодовые базы с равнивать производительность
...
Визуальный отладчик проверки типов должен быть полезен новичкам. Уже есть [аналогичная поделка для Haskell](https://chameleon.typecheck.me/playground), которая поддерживает некоторое подмножество языка Haskell, и делает отладчик типов.
Частичное описание есть в PDF [c этой страницы](https://icfp21.sigplan.org/details/TyDe-2021/6/Interactive-Haskell-Type-Inference-Exploration-Extended-Abstract-).
...
Если аккуратно и глубоко закопаться в теорию, то может получиться учебная практика (или ВКР) на год.
Если получится найти какие-нить отличия OCaml от Haskell при реализации, то можно попробовать написать на местячковую конфу.
...
Планируется для начала взять [игрушечную](https://github.com/Kakadu/unicanren)(она может быть баганая) OCaml реализацию miniKanren, [разобраться](https://github.com/ocaml-multicore/parallel-programming-in-multicore-ocaml) с параллельным программированием в будущей версии OCaml, и скрестить как-нибудь одно с другим.
Во втором можно пытаться добавлять параллелизм в настоящую реализацию [miniKanren](https://github.com/PLTools/OCanren).

Ранее была [проведена некоторая работа](https://github.com/K0lba/unicanren) по интеграции unicanren и OCaml 5.
Несколько реляций можно запустит параллельно, и объединять ответы из них по мере их поступления. Следующим шагов будет добавление параллельной дизъюнкции в представление программ на миниканрене и внесение изменений в интерпретатор.
...
[Как-то сделано](https://github.com/Tozarin/mrm)
